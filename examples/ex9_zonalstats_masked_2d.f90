...
program main
  ...
  write(*,*) "Grid dimensions:", soja%nlons, "x", soja%nlats
  write(*,*) ""

  ! --- Step 1: Unmasked zonal statistics (all states) ---
  call zonalStats(zones, soja, nzones, zcount, zmean, zmin, zmax, zsum, zvar)

  write(*,'(a)') "======================================================================"
  write(*,'(a)') "  BEFORE masking — Soybean Statistics (all states)"
  write(*,'(a)') "======================================================================"
  write(*,'(a20, a10, a12, a14)') "State", "Pixels", "Mean (ha)", "Total (ha)"
  write(*,'(a)') "----------------------------------------------------------------------"

  do z = 1, nzones
    if (zcount(z) > 0 .and. len_trim(uf_name(z)) > 0) then
      write(*,'(a20, i10, f12.2, f14.2)') &
        trim(uf_name(z)), zcount(z), zmean(z), zsum(z)
    end if
  end do
